...
async function addItems(li: Element, listCont: Element, applicationsIds: string[] | undefined) {
    if (applicationsIds && applicationsIds.length > 0) {
        let result: Response;
        let row: IApplication[];
...
function validateCount(li: Element, applicationSize: number) {
    return +li.getAttribute('applications-last-count')! + applicationSize >= 0;
}

function getComplexity(value: string) {
    switch (value) {
        case 'Низкая':
            return 2;
        case 'Средняя':
...
function addItem(li: Element, listCont: Element, options?: IItemOptions) {
    ...
    if (options && options.complexity) {
        const complexityText = getComplexity(options.complexity) as string;
    ...
    buttonApprove.addEventListener('click', async () => {
        ...
        if (listItem.getAttribute('edited') === 'true') {
            ...
            card.setAttribute('applications-last-count',
                (+card.getAttribute('applications-last-count')! + count).toString());
...
async function createListElement(options: ICard) {
    ...
    (li.querySelector('.applications-last-count')! as HTMLElement).innerText = li.getAttribute('applications-last-count')!;
    ...
    card.addEventListener('drop', async (e: DragEvent) => {
        ...
        if (draggedItem && targetList) {
            ...
            validationResult = validateCount(card,
                -Number(getComplexity((<HTMLInputElement>item!.querySelector('.item-complexity p:not(.title)')!).innerText)));
            if (!validationResult) {
